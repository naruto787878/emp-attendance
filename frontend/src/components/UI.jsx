    ...
    >
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 22 }}>
          <span style={{ fontSize: 16, fontWeight: 500, color: '#e8eaf0' }}>{title}</span>
          <button
            onClick={onClose}
            style={{ background: 'none', border: 'none', color: '#4b5563', fontSize: 22, cursor: 'pointer', lineHeight: 1, padding: '0 4px' }}
          >×</button>
        </div>
        <div>{children}</div>
        {footer && (
...
// ── Button ───────────────────────────────────────────────────────
const BTN_STYLES = {
  default: { background: '#1a1f2e', color: '#9ca3af', border: '1px solid #2a2f40' },
  ...
  ghost:   { background: 'transparent', color: '#6b7280', border: '1px solid #2a2f40' },
};

export function Btn({ children, onClick, variant = 'default', size = 'md', disabled, style: extra = {} }) {
  const v = BTN_STYLES[variant] || BTN_STYLES.default;
  return (
    <button
      disabled={disabled}
      onClick={onClick}
    ...
    <div style={{ marginBottom: 16 }}>
      ...
      <input
        ...
        style={{
          ...
          fontSize: 13.5, background: '#0a0d14',
          border: `1px solid ${error ? '#7f1d1d' : focused ? '#2563eb' : '#1e2230'}`,
    ...
    <div style={{ marginBottom: 16 }}>
      ...
      <select
        ...
        style={{
          width: '100%', padding: '9px 12px', borderRadius: 8,
          fontSize: 13.5, background: '#0a0d14',
          border: `1px solid ${focused ? '#2563eb' : '#1e2230'}`,
          color: '#e8eaf0', outline: 'none', cursor: 'pointer',
          transition: 'border-color 0.15s',
        }}
        {...props}
      >
...
export function StatCard({ label, value, sub, color = '#2563eb', icon }) {
  return (
    <div style={{
      ...
      {icon && (
        <div style={{ fontSize: 20, marginBottom: 10, opacity: 0.6 }}>{icon}</div>
      )}
      <div style={{ fontSize: 11.5, color: '#6b7280', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
        {label}
      </div>
      <div style={{ fontSize: 28, fontWeight: 600, color: '#e8eaf0', fontFamily: "'Syne', sans-serif", lineHeight: 1.1 }}>
        {value}
      </div>
      {sub && <div style={{ fontSize: 12, color: '#4b5563', marginTop: 5 }}>{sub}</div>}
    </div>
  );
}
